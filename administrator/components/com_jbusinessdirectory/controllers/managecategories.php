...
class JBusinessDirectoryControllerManageCategories extends JControllerLegacy
{
	...
	function deleteCategory(){
		...
		if($category[0]->parentId == 0){
			...
			$category = $model->getCompleteCategoryById($category[0]->parentId);
	...
	function getCategoryById(){
		...
		echo '<?xml version="1.0" encoding="utf-8" ?>';
		echo '<category_statement>';
		echo '<answer id="'.$category[0]->id.'" parentId="'.$category[0]->parentId.'"  name="'.$category[0]->name.'"  alias="'.$category[0]->alias.'" description="'.$category[0]->description.'" imagePath="'.$category[0]->imageLocation.'" markerPath="'.$category[0]->markerLocation.'"/>';
		echo '</category_statement>';
		echo '</xml>';
		exit;
	}
