...
abstract class modJBusinessLatestHelper
{
	public static function getList(&$params){
		...
		//dump($whereCatCond);
		$query = "select slec.*, mainCategory, mainCategoryId, companyName, companyId1, count(cra.id) as nrRatings, features, GREATEST(if(FIND_IN_SET('featured_companies',features) ,1,0), slec.featured) as featured
				from (
				...
				left join #__jbusinessdirectory_packages p on (inv.package_id=p.id ) or (p.id=cp.package_id and p.price=0)
				left join #__jbusinessdirectory_package_fields pf on p.id=pf.package_id
				where 1  $whereCatCond $packageFilter  and cp.state=1 $companyStatusFilter
