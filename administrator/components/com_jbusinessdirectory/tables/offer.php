...
class JTableOffer extends JTable
{
	...
	function getOffer($offerId){
		$db =JFactory::getDBO();
		$query = "select * from #__jbusinessdirectory_company_offers where id=".$offerId;
		$db->setQuery($query);
		//dump($query);
	...
	function getCompanyOffers($companyId, $limitstart=0, $limit=0){
		...
		$query = " select co.id, co.subject, co.short_description, co.address, co.city, co.county, co.latitude, co.longitude, co.startDate, co.endDate, co.alias, co.view_type, co.article_id, co.url,
	...
	function getOffersByCategoriesSql($categoriesIDs, $enablePackage, $showPendingApproval, $orderBy=null){
		$whereCatCond = '';
		//dump($categoriesIDs);
		...
		op.picture_info,op.picture_path,GROUP_CONCAT( DISTINCT cg.id) as categoryIds, GROUP_CONCAT(cg.name separator '#') as categoryNames, GROUP_CONCAT(cg.alias separator '#') as categoryAliases 
					...
					where co.state=1 and co.approved !=-1 and (co.publish_start_date<=DATE(now()) or co.publish_start_date='0000-00-00' or co.publish_start_date is null) and  (co.publish_end_date>=DATE(now()) or co.publish_end_date='0000-00-00' or co.publish_end_date is null) $whereCatCond $packageFilter $companyStatusFilter 
					and cp.state=1
					and
						(1 or op.id in (
								select  min(op1.id) as min from #__jbusinessdirectory_company_offers co1
	...
	function getOffersByCategories($categoriesIDs, $enablePackage, $showPendingApproval, $orderBy, $limitstart=0, $limit=0){
		$db =JFactory::getDBO();

		$query = $this->getOffersByCategoriesSql($categoriesIDs, $enablePackage, $showPendingApproval,$orderBy);
	...
	function getTotalOffersByCategories($categoriesIDs, $enablePackage, $showPendingApproval){
		$db =JFactory::getDBO();

		$query = $this->getOffersByCategoriesSql($categoriesIDs, $enablePackage, $showPendingApproval);
