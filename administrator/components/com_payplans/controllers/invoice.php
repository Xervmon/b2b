...
class PayplansadminControllerInvoice extends XiController
{
	...
	public function deleteModifier()
	{
	 ...
	 /**
	 * reassignSerialInitialize set up the modal popup 
	 * @since 3.2
	...
	{	
		...
		$query->select('count(`invoice`.`invoice_id`)')
		...
		$view = $this->getView();
		$view->assign('totalRecords', $totalRecords);
		
		return true;
	}
	
	/**
	* reassignSerial update the processed invoices   
	* @since 3.2
	* @return ajax response
	 */
	public function reassignSerial()
	{
		...
		list($start, $needToContinue) = $this->updateReassignSerial($start);
		
		$view = $this->getView();
		
	
		$view->set('needToContinue', $needToContinue);
	...
	/**
	* updateReassignSerial processign the invoices 
	* @since 3.2
	* @return multitype:number boolean
	 */
	public function updateReassignSerial($start = 0)
	{
		$limit  = 30;
		$done = true;
		...
		if ( 0 === (int)$start) {
			...
			$configModel->save(array('expert_invoice_last_serial'=>0));
		}
		
		$done = PayplansHelperInvoice::reassignSerial($start, $limit);
		$start = $start + $limit;

		return array($start, $done);
	}
