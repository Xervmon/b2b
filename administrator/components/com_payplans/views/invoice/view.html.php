...
class PayplansadminViewInvoice extends XiView
{
	protected function _adminGridToolbar()
	{
		...
		//hide the reassignment functionality of invoice serial due to accounting issues
		//XiHelperToolbar::openPopup('reassignSerial', 'pprefresh', 'pprefresh.png', 'COM_PAYPLANS_TOOLBAR_REASSIGN_INVOICE', true );
	}
    ...
    public function reassignSerialInitialize()
    {
    
    	$confirm = XiFactory::getApplication()->input->get('confirm', 0);
    
    	if ( empty($confirm) ) {
    
    		$serialFormat = XiFactory::getConfig()->expert_invoice_serial_format;
    		$invoicePaidOn	= new XiDate('now');
    		...
    		$replace = array(99, $paidDate, $paidMonth, $paidYear, $paidDay);
    		$exampleSerial = str_replace($search, $replace, $serialFormat);
    			
    		$ajaxResponse 	= XiAjaxResponse::getInstance();
    		$ajaxResponse->addScriptCall('xi.ui.dialog.title',XiText::_('COM_PAYPLANS_INVOICE_REASSINING_INVOICE_SERIAL_TITLE'));
    		$ajaxResponse->addScriptCall('xi.ui.dialog.body',XiText::sprintf('COM_PAYPLANS_INVOICE_REASSINING_INVOICE_SERIAL_CONFIRM', $exampleSerial));
    
    		$confirmation 		 	= new stdClass();
    		$confirmation->click   	= 'payplans.jQuery(this).hide(); payplansAdmin.invoice_reassignSerial(1)';
    		$confirmation->text    	= XiText::_('COM_PAYPLANS_AJAX_CONFIRM_BUTTON');
    		$confirmation->classes 	= 'btn btn-primary';
    
    		$cancel 		 = new stdClass();
    		$cancel->click   = 'xi.ui.dialog.close()';
    		$cancel->text 	 = XiText::_('COM_PAYPLANS_AJAX_CANCEL_BUTTON');
    		$cancel->classes = 'btn btn-default';
    			
    		$actions = array($confirmation, $cancel);
    		$ajaxResponse->addScriptCall('xi.ui.dialog.button',$actions);
    	...
    	//$ajaxResponse->addScriptCall('xi.ui.dialog.title',XiText::_('COM_PAYPLANS_INVOICE_REASSINING_INVOICE_SERIAL_TITLE'));
    	$ajaxResponse->addScriptCall('xi.ui.dialog.button',array());
    	$ajaxResponse->addScriptCall('xi.ui.dialog.body',$this->loadTemplate('reassign_serial'));
    	$ajaxResponse->addScriptCall('payplansAdmin.invoice_updateReassignSerial', array('start' => 0, 'needToContinue' => 1));
    	$ajaxResponse->sendResponse();
    
    }
    
    public function reassignSerial()
    {
    	$ajaxResponse = XiAjaxResponse::getInstance();
    	$ajaxResponse->addScriptCall('payplansAdmin.invoice_updateReassignSerial', array('start' => $this->start, 'needToContinue' => $this->needToContinue));
    	$ajaxResponse->sendResponse();
    }
