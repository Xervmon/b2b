...
class PayplansadminViewTransaction extends XiView
{
	...
	{	
		...
		{
			//If payments are recurring then Refund button will be shown on last transaction only
			$invoice = $transaction->getInvoice(PAYPLANS_INSTANCE_REQUIRE);
			if($invoice->isRecurring()){
				$allTransactions	= $invoice->getTransactions();
				$lastTransaction	= array_pop($allTransactions);
