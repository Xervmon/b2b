...
class PayplansHelperUpgrade
{
	...
	protected static function _calculatePaymentsDuringPreviousUpgradations($order)
	{
		...
		if(count($invoices) == 0){
			...
			$totalValue  = PayplansHelperInvoice::totalExcludingModifiers($invoice, null, array(PayplansModifier::PERCENT_OF_SUBTOTAL_TAXABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_DISCOUNTABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_NON_TAXABLE));
		}
		...
		while($upgradedFrom)
		{
			...
			// pick last invoices
			$invoice    = array_pop($invoices);
			$totalValue = $totalValue + PayplansHelperInvoice::totalExcludingModifiers($invoice, null, array(PayplansModifier::PERCENT_OF_SUBTOTAL_TAXABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_DISCOUNTABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_NON_TAXABLE));
			$upgradedFrom = $order->getParam('upgrading_from',0);
