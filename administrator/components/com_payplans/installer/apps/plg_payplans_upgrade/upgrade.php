...
class plgPayplansUpgrade extends XiPlugin
{
	...
	public function onPayplansUpgradeFromRequest($userid = null, $orderId = null)
	{
		...
		$response->addScriptCall('xi.ui.dialog.height','400px');
	...
	public function onPayplansUpgradeDisplayConfirm($newPlanId, $subKey, $userid = null)
	{
		...
		$modifierParams->message	= XiText::_('COM_PAYPLANS_APP_UPGRADE_MESSAGE');
	...
	/**
	 * When user confirms upgrade-order, we need to make sure everything is up-to-date
	 * because - I can make a upgrade request today, if I do not complete order
	...
	{	
		...
		$modifiers 	= $newInvoice->getModifiers(array('type'=>'upgrade'));
		...
		$oldOrder 	= $oldSub->getOrder(PAYPLANS_INSTANCE_REQUIRE);
		
		$result 	= PayplansHelperUpgrade::calculateUnutilizedValue($oldSub);
		
		$modifier 	= array_pop($modifiers);
		$modifier->set('amount', -$result['unutilized']);
		$modifier->save();
	...
	function addAjaxOnDisplayOrder($html, $newInvoice, $oldSub)
	{
		...
		if(XiFactory::getApplication()->isAdmin()){
        	...
        	$object3->id    = "upgrade-payment-free";
        	$object3->classes = 'upgrade-options btn btn-primary';
        	...
        	$object4->id    = "upgrade-payment-offline";
        	...
        	$object5->id    = "upgrade-payment-partial";
        	...
        	$object6->attr = 'style="display:none"';

        	$response->addScriptCall('xi.ui.dialog.button',array($object6, $object3, $object4, $object5, $object1, $object2));
