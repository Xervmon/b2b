...
class  plgPayplansregistrationAuto extends XiPluginRegistration
{
	...
	{
	    $xml = dirname(__FILE__).DS.'auto.xml';
	    if (file_exists($xml)) {
	...
	protected function _doStartRegistration()
	{
		...
		$app 		= PayplansFactory::getApplication();
		$email 		= $app->input->get('payplansRegisterAutoEmail', false, '@');
		$username 	= $app->input->get('payplansRegisterAutoUsername', false, '@');
		$password 	= $app->input->getString('payplansRegisterAutoPassword', false);
