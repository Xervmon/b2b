...
if (!class_exists('plgUserJomSocialUser')) {
    ...
    class plgUserJomSocialUser extends JPlugin {
        ...
        public function deleteFromCommunityWall($user)
        {
            $db = JFactory::getDBO();
            ...
            $sql = "DELETE FROM"
                    ...
                    . "WHERE (
                                ("  . $db->quoteName("contentid") . " = " . $db->quote($user['id'])
                                    . " OR" . $db->quoteName("post_by") . " = " . $db->quote($user['id'])
                                . ")
                                AND" . $db->quoteName("type") . " = " . $db->quote('user')
                            .")"
                        ." OR" . $db->quoteName("post_by") . " = " . $db->quote($user['id']);
            $db->setQuery($sql);
            $db->Query();
