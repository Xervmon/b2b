...
class  PayplansAppEmail extends PayplansApp
{
	...
	protected function _attachInvoice($object)
	{
		...
		if($object instanceof PayplansOrder){
			...
			$invoiceObject 	= array_pop($getInvoices);
		}
		
		//get the invoice object from subscription
		if($object instanceof PayplansSubscription){
			$getOrders		= $object->getOrder(PAYPLANS_INSTANCE_REQUIRE)->getInvoices();
			$invoiceObject 	= array_pop($getOrders);
		}
		...
		if($plgEnable == true){
			...
			$filePath = JPATH_ROOT.DS.'media'.DS.'payplans'.DS.'app'.DS.'pdfinvoice'.DS.'pdfinvoices'.$object->getBuyer();
