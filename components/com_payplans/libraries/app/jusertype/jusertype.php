...
class  PayplansAppJusertype extends PayplansApp
{
	...
	protected function _setJusertype($userid, $group, $subid)
	{
		...
		foreach ($group as $groupid){
			...
			$message = XiText::_("COM_PAYPLANS_APP_JUSERTYPE_LOG_ADD_INTO_USERGROUP");
           	$content = array( 'User Name'=>$userid, 'Usergroup' => $groupid,'Subscription Id'=> $subid );
       		PayplansHelperLogger::log(XiLogger::LEVEL_INFO, $message, $this, $content,'PayplansAppJusertypeFormatter');
		}
	...
	protected function _unsetJusertype($userid, $jusertype, $subid)
	{
		...
		foreach ($jusertype as $group){
			if($this->_removeFromResource($subid, $userid, $group, $this->_resource)){
				...
				$message = XiText::_("COM_PAYPLANS_APP_JUSERTYPE_LOG_REMOVE_FROM_USERGROUP");
           		$content = array( 'User Name'=>$userid, 'Usergroup' => $group,'Subscription Id'=> $subid );
       			PayplansHelperLogger::log(XiLogger::LEVEL_INFO, $message, $this, $content,'PayplansAppJusertypeFormatter');
			}
		}
		return true;
