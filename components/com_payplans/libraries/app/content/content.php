...
class PayplansAppContent extends PayplansApp
{
	...
	private function _checkPlanIntersection($libInstance)
	{
		...
		//when app is set for non-subscriber dashboard and plan subscribe page then no need to check plans
		$location = $this->getAppParam('defined_locations', 0);
		if(strpos($location, 'noaccess') !== false || strpos($location, 'subscribe') !== false){
			return true;
		}
	...
	protected function _processLocations(Array $locations)
	{
		foreach($locations as $location)
		{
			...
			foreach($temp as $value){
				...
				if(($val === 'RANDOM' || $val === 'noaccess') && $actualValue != 'BLANK'){
					continue;
				}
