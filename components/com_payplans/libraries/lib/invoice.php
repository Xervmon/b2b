					...
					implements PayplansIfaceApptriggerable, PayplansIfaceDiscountable, PayplansIfaceMaskable, PayplansIfaceApiInvoice
{
	...
	/**
	 * maintaining the invoice serial number
	 * @var string
	 * @since 3.2
	 */
	...
	public  $_blacklist_tokens = array('params','checked_out_time','checked_out'); 
	...
	public function reset(Array $config=array())
	{
		...
		$this->paid_date 	    = new XiDate('0000:00:00 00:00:00');	
		$this->_payment 		= null;
	...
	{
		$discount = PayplansHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), array(PayplansModifier::FIXED_DISCOUNTABLE, PayplansModifier::PERCENT_DISCOUNTABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_DISCOUNTABLE));
		return PayplansHelperFormat::price($discount);
	}
	...
	public function getTaxableAmount()
		{
			$taxable = PayplansHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), array(PayplansModifier::PERCENT_TAXABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_TAXABLE));
			return PayplansHelperFormat::price($taxable);
		}
	...
	{
		$tax = PayplansHelperModifier::getModificationAmount($this->getSubtotal(), $this->getModifiers(), array(PayplansModifier::FIXED_NON_TAXABLE, PayplansModifier::PERCENT_NON_TAXABLE, PayplansModifier::PERCENT_OF_SUBTOTAL_NON_TAXABLE));
		return PayplansHelperFormat::price($tax);
	}
	...
	*/
	...
	public function displayRefundButton(PayplansPayment $payment)
	{
		...
		if($app->supportForRefund() && $subscription->getStatus() == PayplansStatus::SUBSCRIPTION_ACTIVE){
			return true;
		}
	...
	/**
	* getSerial 
	* if serial number is not yet set then need to create a new one
	* @since 3.2
	* @return string
	 */
	...
	{
		if (null == $this->serial) {
			return XiText::_("COM_PAYPLANS_NOT_APPLICABLE");	
		}
