...
class PayplansRewriter extends JObject
{
	...
	function rewrite($str, $refObject, $requireRelative = true)
	{
		...
		foreach($this->mapping as $key => $value){
			...
			/** we should consider only null and '', cause admin may have some default value like 0, that should be replace.
			*	@since 3.1.5
			*/
			if(!is_array($value) && isset($value) && ($value != null || $value != '')){
			...
			foreach($props as $key => $value){
				...
				if(is_array($value)){
					...
					if(JString::Strtolower($key)== 'status'){
 					...
 					if(($object instanceOf PayplansIfaceMaskable) && JString::Strtolower($key)== 'currency' && method_exists($object, 'getCurrency'))
					{
						$value = $object->getCurrency();
					}
					if (stristr($key, 'date') && $value == null) {
						$value = XiText::_('COM_PAYPLANS_NEVER');
					}
