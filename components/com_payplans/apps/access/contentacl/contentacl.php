...
class PayplansAppContentacl extends PayplansApp
{
		...
		if($block == 'joomla_category' || $block == 'joomla_section' || $block == 'joomla_article'){
			...
			if($block == 'joomla_category'){
				...
				if($this->getParam('applyAll',false)){
					...
					$item->text		= $item->introtext.$links;
				}
			}
	...
	public function getPlanlinks()
	{
		$plans 	   = $this->getPlans();
		$links     = '<strong>'.XiText::_('COM_PAYPLANS_CONTENTACL_SUBSCRIBE_PLAN').'</strong> ( ';
		foreach ($plans as $planId){
			$planName		= PayplansHelperPlan::getName($planId);
			$planLinks[]   	= '<a href="'.XiRoute::_('index.php?option=com_payplans&view=plan&task=subscribe&plan_id='.$planId).'">'.$planName.'</a>';
		}
