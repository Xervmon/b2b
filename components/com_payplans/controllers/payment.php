...
class PayplanssiteControllerPayment extends XiController
{
	...
	public function pay($paymentId = null, $post = null)
	{
		...
		//if payment is not valid then redirect to plan page
		if(empty($paymentId)){
			
        	$this->setMessage(XiText::_('COM_PAYPLANS_ERROR_INVALID_PAYMENT_ID'));
            $this->setRedirect(XiRoute::_("index.php?option=com_payplans&view=dashboard"));
