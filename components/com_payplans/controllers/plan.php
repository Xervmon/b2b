...
class PayplanssiteControllerPlan extends XiController
{
	...
	function login($planId = null, $userId = null)
	{
		...
		if($planId === null || PayplansHelperPlan::isValidPlan($planId) == false){
			$this->setMessage(XiText::_('COM_PAYPLANS_PLAN_PLEASE_SELECT_A_VALID_PLAN'));
		...
		if($application->input->post->get('payplansLoginSubmit', false) != false){
			// check for user name and password
			// XITODO : use it in JAVASCRIPT validation
			$username = $application->input->get('payplansLoginUsername', false,'@');			
			$password = $application->input->getString('payplansLoginPassword', false);
