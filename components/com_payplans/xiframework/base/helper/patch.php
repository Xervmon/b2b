...
class XiHelperPatch extends XiAbstractHelperPatch
{
	...
	static function updateVersion()
	{
		...
		$query[]	= 'UPDATE #__payplans_support'
				  .' SET '. $db->quoteName('value') .' = '.$db->Quote('5').' WHERE '. $db->quoteName('key') .' = '.$db->Quote('build_version');

		$query[] = 'UPDATE #__payplans_support'
				  .' SET '. $db->quoteName('value') .' = '.$db->Quote('3.2').' WHERE '. $db->quoteName('key') .' = '.$db->Quote('global_version');
