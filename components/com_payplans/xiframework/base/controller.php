...
abstract class XiController extends XiAbstractController
{
	...
	public function save()
	{
		...
		$msgType	=	'message';
		$itemId 	= $this->_getId();
		//the id is saved in another variable because, if the table store another data, then this id gets changed
		//and it redirect to another id. For example, assign plan app, assign a new plan, thus it redirect to new subscription
		//rather then old one
		$item_id    =  $itemId;
		...
		if(JRequest::getVar('task')==='apply'){
			...
			$item_id    = empty($item_id)? $table->$keyName : $item_id;
			$redirect  .= "&task=edit&id={$item_id}";
		}
...
//	}
	...
	public function update()
	{
		...
		$data  	= array($name => $value, 'updated'=> true);
