...
class XiLib extends JObject
{
	...
	static public function getInstance($name, $id=0, $type=null, $bindData=null)
	{
		...
		if(!$id){
			...
			return $bindData ? $class_instance->bind($bindData): $class_instance;
		}
		...
		return  $bindData 	? self::$instance[$name][$id]->bind($bindData)
	...
	{
		if(!($object instanceof XiLib) || !$object->getId()){
			throw new Exception('XiFramework : Invalid instance of Lib to set');
		}
	...
	public function toArray($strict = false, $readOnly = false)
	{
		...
		foreach($arr as $key => $value)
		{
			...
			if(is_object($this->$key)){	
				...
				if($this->$key instanceof XiDate && $readOnly) {
					$ret[$key] = PayplansHelperFormat::date($this->$key);
					continue;
				}
