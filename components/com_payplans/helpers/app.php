	...
	static public function getXml()
	{
		...
		if(self::$xmlData === null){
			...
			if (file_exists($xml)) {
				...
				self::$xmlData[$appInstance->getName()]['icon'] = JPATH_ROOT.DS.'administrator'.DS.'components'.DS.'com_payplans'.DS.'templates'.DS.'default'.DS.'_media'.DS.'images'.DS.'icons'.DS.'48'.DS.'app.png';
	...
	public static function getPlugins()
	{
        ...
        foreach($plugins as $plugin ){
        	self::$pluginXmlData[$plugin['name']]['tags'] = array('all');
           	self::$pluginXmlData[$plugin['name']]['icon'] = JUri::root().'administrator'.DS.'components'.DS.'com_payplans'.DS.'templates'.DS.'default'.DS.'_media'.DS.'images'.DS.'icons'.DS.'48'.DS.'app.png';
	       	...
	       	foreach ($plugin['value'] as $element=> $value){
                ...
                self::$pluginXmlData[$plugin['name']][$element] = $value;
                     if($element == 'name')
              	 	...
              	 	if($element == 'icon'){
              	 		self::$pluginXmlData[$plugin['name']][$element]  = JUri::root().'plugins'.DS.(string)$attributes['group'].DS.$plugin['name'].DS.$value;
              	 	}
             	if($element == 'tags'){
