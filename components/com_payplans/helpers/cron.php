   ...
   //IMP:-IGNORE THIS BUG:-IF SUBSCRIPTION GET EXPIRE,THEN STILL IT WILL WAIT FOR GRACE PERIOD TO EXPIRE.
	static function doSubscriptionExpiry(XiDate $time=null)
	{
		...
		foreach($subscriptions as $sub_id => $subscription){
			...
			if($subscription->isRecurring() == false || !self::_isRequiredToProcess($subscription)){
	...
	public static function _isRequiredToProcess($subscription)
	{		
		$recurrenceCount = $subscription->getRecurrenceCount();
		$recurring  	 = $subscription->isRecurring();
		$invoiceCount	 = $subscription->getOrder(true)->getRecurringInvoiceCount();
