...
class JBusinessDirectoryModelManageCompanies extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'bc.*'));
		$query->from($db->quoteName('#__jbusinessdirectory_companies').' AS bc');
		
		// Join over the company types
		$query->select('ct.name as typeName');
		$query->join('LEFT', $db->quoteName('#__jbusinessdirectory_company_types').' AS ct ON bc.typeId=ct.id');
		
		// Join over the company types
		$query->select('cnt.country_name as country');
		$query->join('LEFT', $db->quoteName('#__jbusinessdirectory_countries').' AS cnt ON bc.countryId=cnt.country_id');
		
		// Filter by search in title.
		$search = $this->getState('filter.search');
		if (!empty($search)) {
			if (stripos($search, 'r:') === 0) {
				$query->where('bc.registrationCode = '.(int) substr($search, 2));
			}
			else {
		...
		$query->where('bc.userId ='.$user->id);
		
		$typeId = $this->getState('filter.type_id');
		if (is_numeric($typeId)) {
			$query->where('bc.typeId ='.(int) $typeId);
		}
		
		$statusId = $this->getState('filter.status_id');
		if (is_numeric($statusId)) {
			$query->where('bc.approved ='.(int) $statusId);
		}
		
		$stateId = $this->getState('filter.state_id');
		if (is_numeric($stateId)) {
			$query->where('bc.state ='.(int) $stateId);
		}
		
		$query->group('bc.id');

		// Add the list ordering clause.
		$query->order($db->escape($this->getState('list.ordering', 'bc.id')).' '.$db->escape($this->getState('list.direction', 'ASC')));
