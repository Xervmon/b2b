...
class BriefcaseFactoryFrontendModelPrivate extends FactoryModelList
{
  protected
  ...
  public function getParent($id = null)
  {
    ...
    if (!isset($parents[$id])) {
      ...
      // Check if folder is private shared.
      if (!$table->isRoot() && !$table->isSharedWithUser(JFactory::getUser()->id)) {
        throw new Exception(FactoryText::sprintf('private_folder_not_shared', $id), 403);
      }
  ...
  public function getBreadcrumbs()
  {
    ...
    foreach ($parent->getPath() as $folder) {
      ...
      if ($table->isRoot() || $table->isSharedWithUser()) {
        $breadcrumbs[] = $folder;
  ...
  public function getQueryForFolders($folder_id, $total = false, $userId = null)
  {
    ...
    if ($parent->isRoot()) {
      // If parent folder is root, show all the folders that have been explicitly shared with the user.
      ...
      // Since we are allowed access to this folder, we must assume that the folder is already privately shared.
  ...
  public function getQueryForFiles($folder_id, $total = false, $userId = null)
  {
    ...
    $parent = $this->getParent();
    if ($parent->isRoot()) {
      // If parent folder is root, show all the files that have been explicitly shared with the user.
      $nullDate = $dbo->getNullDate();
      $now      = JFactory::getDate()->toSql();
      $groups   = JAccess::getGroupsByUser($userId);
