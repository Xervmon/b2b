...
//			$url = $handler->getEditedAlbumURL( $albumid );
    ...
    public function removealbum() {
        ...
        if ($album->delete()) {
            ...
            // Remove from activity stream
            CActivityStream::remove('albums', $id);

            // Remove likes activity
            $likeModel = CFactory::getModel('like');
            $likeModel->removeLikes('album', $id);

            // Remove comment
            $wallModel = CFactory::getModel('wall');
            $wallModel->deletePostByType('albums', $id);
...
// 			            $image->rotateImage($public, 90);
    ...
    public function changeAvatar() {
        ...
        // Cache leftovers.
        $oldAvatar    = $cTable->avatar;
        $oldTypeImg   = preg_replace('/([0-9a-f]+)\./', $type . '-$1.', $oldAvatar);
        $oldStreamImg = preg_replace('/([0-9a-f]+)\./', '$1_stream_.', $oldAvatar);
        ...
        // Remove leftovers.
        JFile::delete($oldTypeImg);
        JFile::delete($oldStreamImg);
    ...
    public function ajaxUpdateThumbnail($type, $id, $sourceX, $sourceY, $width, $height) {
       ...
       $objResponse = new JAXResponse();
        ...
        $objResponse->addScriptCall('window.location.assign("' . str_replace('&amp;', '&', $url) . '")');
        return $objResponse->sendResponse();
    }
