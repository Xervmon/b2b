    ...
    public function getEvents($categoryId = null, $userId = null, $sorting = null, $search = null, $hideOldEvent = true, $showOnlyOldEvent = false, $pending = null, $advance = null, $type = CEventHelper::ALL_TYPES, $contentid = 0, $limit = null) {
        ...
        if (!empty($advance)) {
            if (!empty($advance['startdate'])) {
                $startDate = date('Ymd', strtotime($advance['startdate']));
                $extraSQL .= ' AND DATE_FORMAT(a.' . $db->quoteName('startdate') . ',"%Y%m%d") >= ' . $db->Quote($startDate);
            ...
            if (!empty($advance['enddate'])) {
                $endDate = date('Ymd', strtotime($advance['enddate']));
                $extraSQL .= ' AND DATE_FORMAT(a.' . $db->quoteName('startdate') . ',"%Y%m%d") <= ' . $db->Quote($endDate);
            }
