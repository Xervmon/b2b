        ...
        public function runConvert()
        {
            ...
            if ($zencoder)
            {
                ...
                foreach ($videos as $video) {
                    ...
                    //$outputObj->setNotificationUrl(CRoute::getExternalURL('index.php?option=com_community&view=videos&task=zencodercallback&videoid='.$video->id.'&videoname='.$videoName[count($videoName)-1])); //callback for zencoder
    ...
    // 		$data[]	= '-metadatacreator:' . 'Jomsocial.com';
            ...
            if (! empty($videoLink))
            {
                ...
                if ( !empty($matches['domain']))
                {
                    ...
                    if($providerName == 'yahoo'
                            && strpos($parsedLink['host'], 'video') === false
                            && strpos($parsedLink['host'], 'screen') === false) {
                        $providerName = 'invalid';
                    }
                }
        ...
        static public function getActivityTitleHTML($act)
        {
            ...
            if($param->get('style') == COMMUNITY_STREAM_STYLE || empty($act->content) && !is_null($groupTitle) ){
                $html .= JText::sprintf('COM_COMMUNITY_ACTIVITY_VIDEO_SHARE_TITLE',CRoute::_($act->params->get('video_url')),$groupTitle);
            } else if($act->app == "videos.linking"){
                $html .= JText::sprintf('COM_COMMUNITY_ACTIVITY_VIDEO_LINK_TITLE',CRoute::_($act->params->get('video_url')));
            } else if($param->get('action', 'upload')) {
                $html .= JText::sprintf('COM_COMMUNITY_ACTIVITY_VIDEO_UPLOAD_TITLE',CRoute::_($act->params->get('video_url')));
            }
