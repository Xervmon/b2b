...
joms.extend({
    ...
    invitation: {
        ...
        selectNone: function(listID) {
            joms.jQuery(listID).find('li').each(function() {
                joms.jQuery(this).remove();
                joms.jQuery(this).find('input').removeAttr('checked');
                joms.jQuery('#community-invitation').find('input').removeAttr('checked');
            });
        },
    ...
    photos: {
        ...
        // Defer execution to save resource.
        previewThumbTimer: false,
        previewThumb: function(img, selection) {
            window.clearTimeout( joms.photos.previewThumb );
            joms.photos.previewThumb = window.setTimeout(function() {
                var scaleX     = 64 / (selection.width || 1),
                    ...
                    image      = joms.jQuery('#thumb-hold > img');
    ...
    connect: {
        ...
        update: function() {
            var token = window.FB && FB.getAccessToken && FB.getAccessToken();
            var ajaxCall = "jax.call('community', 'connect,ajaxUpdate'" + ( token ? (", '" + token + "'") : "" ) + " );";
            cWindowShow(ajaxCall, '', 450, 200);
        },
    ...
    videos: {
        ...
        submitLinkVideo: function() {
            ...
            var videoLinkUrl = joms.jQuery('#linkVideo input[name=videoLinkUrl]');
            if ( !joms.jQuery.trim(videoLinkUrl.val()) ) {
                videoLinkUrl.addClass('invalid');
                videoLinkUrl.siblings('#videoLinkUrlError').show();
                isValid = false;
            } else {
                videoLinkUrl.removeClass('invalid');
                videoLinkUrl.siblings('#videoLinkUrlError').hide();
            }

            var videoCategory = joms.jQuery('#linkVideo select[name=category_id]');
            if ( !videoCategory.val() ) {
                videoCategory.addClass('invalid');
                videoCategory.siblings('#videoCategoryError').show();
                isValid = false;
            } else {
                videoCategory.removeClass('invalid');
                videoCategory.siblings('#videoCategoryError').hide();
            }
        ...
        submitUploadVideo: function() {
            ...
            var videoFile = joms.jQuery('#uploadVideo input[name=videoFile]');
            if ( !joms.jQuery.trim(videoFile.val()) ) {
                videoFile.addClass('invalid');
                isValid = false;
            } else {
                videoFile.removeClass('invalid');
            }

            var videoTitle = joms.jQuery('#uploadVideo input[name=title]');
            if ( !joms.jQuery.trim(videoTitle.val()) ) {
                videoTitle.addClass('invalid');
                videoTitle.siblings('#videoTitleError').show();
                isValid = false;
            } else {
                videoTitle.removeClass('invalid');
                videoTitle.siblings('#videoTitleError').hide();
            }

            var videoCategory = joms.jQuery('#uploadVideo select[name=category_id]');
            if ( !videoCategory.val() ) {
                videoCategory.addClass('invalid');
                videoCategory.siblings('#videoCategoryError').show();
                isValid = false;
    ...
    file: {
        ...
        {
            joms.jQuery('.cTabContainer ul').filter(':visible').html( files );
        },
