...
},uploadVideo:function(b,a){var c="jax.call('community', 'videos,ajaxUploadVideo', '"+b+"', '"+a+"');";cWindowShow(c,"",450,500);}},activities:{infiniteScrollLoaded:0,infinitesScrollenable:1,init:function(){joms.jQuery("#activity-stream-container").on("click",".dropdown-menu [data-action=delete]",function(b){b.preventDefault();
var a=joms.jQuery(b.target).closest("[data-streamid]");joms.activities.remove("",a.data("streamid"));});joms.activities.initMap();joms.activities.loadNewStream();
if(joms.activities.infinitesScrollenable&&!joms.activities.infiniteScrollLoaded){joms.activities.infiniteScroll();}},showMap:function(e,d){if(joms.jQuery("li[data-streamid="+e+"]").find(".cMapHeatzone").length==0){var b=joms.jQuery("li[data-streamid="+e+"]").find(".cStream-Actions").width();
...
}var c=f.data("filter");var a=f.data("filterid");joms.jQuery("#activity-more .btn").hide();joms.jQuery("#activity-more .loading").show();jax.call("community","activities,ajaxGetOlderActivities",b,c,a,f.data("filter-value"));
...
}joms.jQuery(a).children("li").appendTo(".cMain #activity-stream-container > ul.list-unstyled");container.find("#activity-more .loading").hide();if(!joms.activities.infiniteScrollLoaded){joms.jQuery("#activity-more .btn").show();
}else{setTimeout(function(){joms.activities.infiniteScrollDelay=0;},1000);}},loadNewStream:function(){if(joms.jQuery(".joms-latest-activities-container").data("actid")>-1){joms.jQuery(".joms-latest-activities-container").css("display","none");
...
}else{joms.jQuery(".joms-latest-activities-container").hide();}if(a!=0){setTimeout("joms.activities.loadNewStream()",a);}},appendNewStream:function(b,a){joms.jQuery(".joms-latest-activities-container").hide().empty();
joms.jQuery(b).children("li").prependTo(".cMain #activity-stream-container > ul.list-unstyled");},showNewStream:function(){var a=joms.jQuery("#activity-stream-container ul.list-unstyled");
...
cWindowShow(b,"",450,100);},infiniteScroll:function(){if(joms.activities.infiniteScrollLoaded){return;}var b=navigator.userAgent.toLowerCase();var a=!!b.match(/android|webos|iphone|ipad|ipod|blackberry|iemobile|opera mini/i);
if(a){return;}var d=joms.jQuery(window),c=joms.jQuery(document);joms.jQuery("#activity-more .btn").show();d.on("scroll",function(){if(d.scrollTop()==c.height()-d.height()){if(joms.activities.infiniteScrollDelay){return;
}joms.activities.infiniteScrollDelay=1;joms.activities.more();setTimeout(function(){joms.activities.infiniteScrollDelay=0;},10000);}});joms.activities.infiniteScrollLoaded=1;
...
var a=520;a=d!="0"?a:a-108;a=b!="0"?a:a-108;cWindowShow(c,"",550,a);},send:function(h,g){var d=joms.jQuery("#community-invitation-form"),a=d.serializeArray(),c=[],f=[];
if(a&&a.length){for(var b=0,e;b<a.length;b++){e=a[b];if(e.name==="friends[]"&&c.indexOf(e.value)<0){c.push(e.value);}}}f.push(["friendsearch",d.find("[name=friendsearch]").val()]);
f.push(["emails",d.find("[name=emails]").val()]);f.push(["message",d.find("[name=message]").val()]);c.length&&f.push(["friends",c.join(",")]);jax.call("community","system,ajaxSubmitInvitation",h,g,f);
...
},selectNone:function(a){joms.jQuery(a).find("li").each(function(){joms.jQuery(this).remove();joms.jQuery(this).find("input").removeAttr("checked");joms.jQuery("#community-invitation").find("input").removeAttr("checked");
});},selectAll:function(a){joms.jQuery(a).find("li").each(function(){joms.jQuery(this).find("input").attr("checked","checked");if(joms.jQuery(this).find("input").attr("checked")){joms.invitation.selectMember("#"+joms.jQuery(this).attr("id"));
}});}},memberlist:{submit:function(){if(joms.jQuery("input#title").val()==""){joms.jQuery("#filter-title-error").show();return false;}if(joms.jQuery("textarea#description").val()==""){joms.jQuery("#filter-description-error").show();
return false;}joms.jQuery("#jsform-memberlist-addlist").submit();},showSaveForm:function(k,b){var k=k.split(",");var j=Array();var e=joms.jQuery("#avatar:checked").val()!=1?0:1;
for(var c=0;c<k.length;c++){var l=new Array();var g="";var h=k[c];if((b["fieldType"+h]=="date"||b["fieldType"+h]=="birthdate")&&b["condition"+h]=="between"){g=b["value"+k[c]]+","+b["value"+k[c]+"_2"];
...
joms.jQuery("#"+d.files[b].id+".plupload_done .plupload_file_name").prepend(c);}var a=joms.jQuery("#albumid option:selected").html();var e=joms.photos.multiUpload.uploadedCompleteMsg.replace("%1$s","<strong>"+a+"</strong>");
...
cWindowShow(a,"",450,100);},ajaxUpload:function(c,d,b){if(joms.photos.ajaxUpload.uploading){return;}joms.photos.ajaxUpload.uploading=true;var a=joms.jQuery("#jsform-uploadavatar").prop("action");
joms.jQuery.ajaxFileUpload({url:a,secureuri:false,fileElementId:"filedata",dataType:"json",beforeSend:function(){},complete:function(){joms.photos.ajaxUpload.uploading=false;
},success:function(k,f){if(k.error=="true"){joms.jQuery("span.error").remove();joms.jQuery("#avatar-upload").prepend('<span class="error">'+k.msg+"</span>");
return false;}else{a=k.msg;joms.jQuery("span.error").remove();var h="";joms.jQuery("#thumb-crop").css("min-height",0);joms.jQuery("#large-avatar-pic").prop("src",a);
joms.jQuery("#thumb-hold > img").prop("src",a);joms.jQuery("div.status-anchor img").prop("src",a);joms.jQuery("#large-avatar-pic").load(function(){var m=Math.round((64/160)*joms.jQuery("#large-avatar-pic").width());
...
var b=c.getSelection();jax.call("community","photos,ajaxUpdateThumbnail",a,d,b.x1,b.y1,b.width,b.height);},previewThumbTimer:false,previewThumb:function(a,b){window.clearTimeout(joms.photos.previewThumb);
joms.photos.previewThumb=window.setTimeout(function(){var e=64/(b.width||1),d=64/(b.height||1),f=Math.round(e*joms.jQuery("#large-avatar-pic").width())+"px",c=Math.round(d*joms.jQuery("#large-avatar-pic").height())+"px",h="-"+Math.round(e*b.x1)+"px",g="-"+Math.round(d*b.y1)+"px",j=joms.jQuery("#thumb-hold > img");
j.css({position:"absolute",width:f,height:c,left:h,top:g});},30);},ajaxRemoveImgSelect:function(){joms.jQuery(".imgareaselect-selection").parent().remove();
...
}var j=window.location.hash||"",c=window.location.href,d;c=c.replace(j,"");if(j&&j.match(/^#photoid=\d+$/)){d=j.replace("#photoid=","");}if(!d){d=c.match(/.+photoid=(\d+).*/);
d=d&&d[1];}if(!d){d=joms.photos.photoSlider.controlObj.find(l).eq(0).attr("id");}joms.photos.photoSlider.controlObj.find("img").removeClass(joms.photos.photoSlider.img_thumbClass);
joms.photos.photoSlider.controlObj.find('img[id="'+d+'"]').addClass(joms.photos.photoSlider.img_thumbClass);var l=joms.jQuery("img#photoSlider_thumb"+d).css("opacity",joms.photos.photoSlider.fullOpacity);
joms.jQuery(".image_thumb").not(l).css("opacity",joms.photos.photoSlider.partialOpacity);if(!l.attr("src")&&l.data("src")){l[0].src=l.data("src");}var m=l;
for(var k=0;k<6;k++){m=m.closest("li").next("li");if(!m.length){break;}m=m.find("img");if(m.length&&!m.attr("src")&&m.data("src")){m[0].src=m.data("src");
}}var f=l;for(var k=6;k>0;k--){f=f.closest("li").prev("li");if(!f.length){break;}f=f.find("img");if(f.length&&!f.attr("src")&&f.data("src")){f[0].src=f.data("src");
}}var p=l.closest("li"),o=(p.position()||{}).left,n=p.closest("ul"),b=(n.position()||{}).left,h=n.width(),a=n.closest("div"),e=a.width(),g=e/2-p.outerWidth(true)/2;
if(h<e){b=e/2-h/2;}else{b=Math.max(0-(h-e),Math.min(0,0-o+g));}n.stop().animate({left:b});},updateThumb:function(c,a){var b='img[id="'+joms.photos.photoSlider.img_thumbId+c+'"]';
...
}jax.loadingFunction=function(){};jax.call("community","system,ajaxLoadFriendsList",b,e,d,c,a);},loadMoreFriend:function(d,c,b,a){name=joms.jQuery("#friend-search-filter").val();
joms.jQuery("#community-invitation-list").addClass("load-more");jax.call("community","system,ajaxLoadFriendsList",name,d,c,b,a);},showForm:function(g,f,e,d,b){var c='jax.call("community", "system,ajaxShowFriendsForm","'+g+'","'+f+'","'+e+'","'+d+'","'+b+'")';
...
}},miniwall:{initialize:function(){var a=joms.jQuery("#activity-stream-container");joms.jQuery('a[href="#removeComment"]').on("click",function(){var b=joms.jQuery(this).parents("[data-commentid]").data("commentid");
joms.miniwall.remove(b);});a.on("click",'a[href="#showallcomments"]',function(d){d.preventDefault();var b=joms.jQuery(this);if(b.data("loading")){return;
}b.data("loading",1);var c=b.closest("li").data("streamid");jax.call("community","system,ajaxStreamShowComments",c);});joms.jQuery(".cStream-Form textarea").width(joms.jQuery(".cStream-Form").width()-12);
...
}else{b=joms.jQuery(".cMain").find("#wall-cmt-"+e);}var a=b.find("textarea");if(!a.data("inittextntags")){joms.miniwall._add(e,b,a.val());return;}a.textntags("val",function(f){joms.miniwall._add(e,b,f);
...
}},autogrow:function(b,a){if(a==undefined){a={};}a.maxHeight=a.maxHeight||300;joms.jQuery(b).autogrow(a);}},map:{queue:[],execute:function(a){if(window.google&&window.google.maps&&window.google.maps.places){a();
return;}if(joms.map.loading){joms.map.queue.push(a);return;}joms.map.loading=true;joms.map.queue.push(a);joms.map.loadScript();},loadScript:function(){var a=document.createElement("script");
...
},loadScriptCallback:function(){if(joms.map.queue&&joms.map.queue.length){while(joms.map.queue.length){(joms.map.queue.shift())();}}joms.map.loading=false;
}},maps:{mapsObj:null,geocoder:null,init:function(){if(joms.jQuery(".cMapFade")!=null||joms.jQuery(".cMapHeatzone")!=null){joms.jQuery(".cMapFade").live("mouseover",function(a){joms.jQuery(this).find("img:eq(2)").fadeOut(0);
...
});joms.jQuery(".cMapHeatzone").live("mouseout",function(a){joms.jQuery(this).parent().find("img:eq(1)").fadeIn(0);});}},initialize:function(c,a,d,b){joms.map.execute(function(){if(typeof google.maps=="undefined"){setTimeout("joms.maps.initialize('"+c+"', '"+a+"')",1000);
...
}var l=new google.maps.LatLng(-34.397,150.644);var g={zoom:14,center:l,mapTypeId:google.maps.MapTypeId.ROADMAP,mapTypeControl:false,disableDefaultUI:true,draggable:false,scaleControl:false,scrollwheel:false,navigationControl:false,streetViewControl:false,disableDoubleClickZoom:true};
var h=joms.maps.mapsObj.length;joms.maps.mapsObj[h]=new google.maps.Map(document.getElementById(c),g);joms.maps.mapsObj[h].setCenter(j[0].geometry.location);
var e=new google.maps.Marker({map:joms.maps.mapsObj[h],position:j[0].geometry.location,title:d});if(b.length>0){var k=new google.maps.InfoWindow({content:b});
google.maps.event.addListener(e,"click",function(){var m=joms.jQuery("div#"+c).data("maps");k.open(joms.maps.mapsObj[m],e);});}joms.jQuery("div#"+c).data("maps",h);
}else{alert("Geocode was not successful for the following reason: "+f);}});}});},addMarker:function(d,c,a,e,b){joms.map.execute(function(){if(joms.maps.mapsObj==null){setTimeout("joms.maps.addMarker('"+d+"', "+c+", "+a+", '"+e+"', '"+b+"')",1000);
}else{var h=joms.jQuery("div#"+d).data("maps");var g=new google.maps.LatLng(c,a);var f=new google.maps.Marker({position:g,map:joms.maps.mapsObj[h],title:e});
if(b.length>0){var j=new google.maps.InfoWindow({content:b});google.maps.event.addListener(f,"click",function(){var k=joms.jQuery("div#"+d).data("maps");
j.open(joms.maps.mapsObj[k],f);});}}});}},connect:{checkRealname:function(a){var b=jax.loadingFunction;jax.loadingFunction=function(){};jax.doneLoadingFunction=function(){jax.loadingFunction=b;
...
};jax.call("community","connect,ajaxCheckUsername",a);},update:function(){var a=window.FB&&FB.getAccessToken&&FB.getAccessToken();var b="jax.call('community', 'connect,ajaxUpdate'"+(a?(", '"+a+"'"):"")+" );";
cWindowShow(b,"",450,200);},updateEmail:function(){joms.jQuery("#facebook-email-update").submit();},importData:function(){var b=joms.jQuery("#importstatus").is(":checked")?1:0;
var a=joms.jQuery("#importavatar").is(":checked")?1:0;jax.call("community","connect,ajaxImportData",b,a);},mergeNotice:function(){var a="jax.call('community','connect,ajaxMergeNotice');";
cWindowShow(a,"",450,200);},merge:function(){var a="jax.call('community','connect,ajaxMerge');";cWindowShow(a,"",450,200);},validateUser:function(){var a="jax.call('community','connect,ajaxValidateLogin','"+joms.jQuery("#existingusername").val()+"','"+joms.jQuery("#existingpassword").val()+"');";
...
}if(joms.jQuery("#newemail").val()==""||joms.jQuery("#error-newemail").css("display")!="none"){b=false;}if(joms.jQuery(".jsProfileType").length>0&&joms.jQuery(".jsProfileType input:checked").length==0){jax.call("community","connect,ajaxCheckProfileType");
b=false;}if(b){var a="";if(joms.jQuery(".jsProfileType").length>0&&joms.jQuery(".jsProfileType input").length>0){a=(joms.jQuery(".jsProfileType input:checked").length>0)?joms.jQuery(".jsProfileType input:checked").val():a;
...
},uploadVideo:function(b,a){var c="jax.call('community', 'videos,ajaxUploadVideo', '"+b+"', '"+a+"');";cWindowShow(c,"",450,100);},submitLinkVideo:function(){var b=true;
var a=joms.jQuery("#linkVideo input[name=videoLinkUrl]");if(!joms.jQuery.trim(a.val())){a.addClass("invalid");a.siblings("#videoLinkUrlError").show();b=false;
}else{a.removeClass("invalid");a.siblings("#videoLinkUrlError").hide();}var c=joms.jQuery("#linkVideo select[name=category_id]");if(!c.val()){c.addClass("invalid");
c.siblings("#videoCategoryError").show();b=false;}else{c.removeClass("invalid");c.siblings("#videoCategoryError").hide();}if(b){joms.jQuery("#cwin-wait").css("margin-left","20px");
joms.jQuery("#cwin-wait").show();document.linkVideo.submit();}},submitUploadVideo:function(){var c=true;var b=joms.jQuery("#uploadVideo input[name=videoFile]");
if(!joms.jQuery.trim(b.val())){b.addClass("invalid");c=false;}else{b.removeClass("invalid");}var a=joms.jQuery("#uploadVideo input[name=title]");if(!joms.jQuery.trim(a.val())){a.addClass("invalid");
a.siblings("#videoTitleError").show();c=false;}else{a.removeClass("invalid");a.siblings("#videoTitleError").hide();}var d=joms.jQuery("#uploadVideo select[name=category_id]");
if(!d.val()){d.addClass("invalid");d.siblings("#videoCategoryError").show();c=false;}else{d.removeClass("invalid");d.siblings("#videoCategoryError").hide();
}if(c){joms.jQuery("#cwin-wait").css("margin-left","20px");joms.jQuery("#cwin-wait").show();document.uploadVideo.submit();}},fetchThumbnail:function(a){var b="jax.call('community' , 'videos,ajaxFetchThumbnail', '"+a+"','myvideos');";
cWindowShow(b,"",450,150);},toggleSearchSubmenu:function(a){joms.jQuery(a).next("ul").toggle().find("input[type=text]").focus();},selectVideoTagFriends:function(a){joms.jQuery("#community-invited-list li").each(function(c){var b=joms.jQuery(this).attr("id");
...
});},checkSize:function(d){var c=joms.jQuery("form#uploadVideo"),b=c.find(d),a=c.find("button");a.prop("disabled",true);a=b.siblings(".btn");b=b.val();
b=b.replace(/\\/g,"/").split("/");a.html(b.pop());if(joms.jQuery.browser.msie){joms.jQuery("form#uploadVideo button").prop("disabled",false);return false;
}else{Size=d.files[0].size;}jax.loadingFunction=function(){};jax.call("community","videos,ajaxCheckFileSize",Size);}},users:{banUser:function(a,b){var c="jax.call('community', 'profile,ajaxBanUser', '"+a+"' , '"+b+"');";
...
if(joms.events.submitRSVP.processing){return false;}d.append('<div class="noclick" style="position:absolute;top:0;right:0;left:0;bottom:0" />');setTimeout(function(){var e="Processing...";
if(joms.lang&&joms.lang.processing){e=joms.lang.processing;}c.html(e);c.removeAttr("class");joms.events.submitRSVP.processing=true;joms.ajax.call("events,ajaxUpdateStatus",[f,b.val()],{success:function(k){joms.events.submitRSVP.processing=false;
joms.jQuery("#community-event-members").replaceWith(k);c.attr("class",b.find("option:selected").attr("class"));c.html(j);d.children(".noclick").remove();
...
}},geolocation:{showNearByEvents:function(a){joms.map.execute(function(){joms.jQuery("#community-event-nearby-listing").show();joms.jQuery("#showNearByEventsLoading").show();
if(typeof(a)=="undefined"){if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(b){var d=b.coords.latitude;var c=b.coords.longitude;
geocoder=new google.maps.Geocoder();var e=new google.maps.LatLng(d,c);geocoder.geocode({latLng:e},function(g,f){if(f==google.maps.GeocoderStatus.OK){if(g[4]){b=g[4].formatted_address;
...
},ajaxUploadFile:function(b,e,d){var a=false;var c=joms.file.___uploader=joms.jQuery("#html5_uploader").pluploadQueue({runtimes:"html5,html4",url:b,max_file_size:d+"mb",chunk_size:d+"mb",unique_names:true,filters:[{title:"Media files",extensions:e}],preinit:{FileUploaded:function(k,g,f){try{var j=joms.jQuery.parseJSON(f.response);
...
joms.jQuery("#cWindowContent").prepend("<strong>"+j.msg+" </strong>");}},UploadComplete:function(f){setTimeout("joms.file.enableEditName();",1000);a=true;
...
joms.jQuery(".plupload_buttons").show();joms.jQuery(".plupload_file_status").hide();joms.jQuery("div#upload-footer").hide();cWindowResize();});}}});if(c.pluploadQueue().runtime=="html4"){joms.file.ieUploadButtonFix();
}joms.jQuery(".plupload_header").remove();joms.jQuery("span.plupload_upload_status").remove();joms.jQuery("input:file").parent().css("top","245px");joms.jQuery("#cwin_close_btn").click(function(){if(a){window.location.reload();
}});},ieUploadButtonFix:function(){var e=joms.file.___uploader;if(!e){return;}var b=e.find(".plupload_add");var d=e.find("form");var a=d.find("input[type=file]");
if(!b.length||!a.length){return;}b.wrap("<span/>");var c=b.parent();c.css({display:"inline-block",position:"relative",overflow:"hidden"});d.appendTo(c);
setTimeout(function(){d.css({margin:0,padding:0,position:"absolute",left:0,top:0,right:"",bottom:"",width:"100%",height:"100%",overflow:"hidden"});a.css({margin:0,padding:0,position:"absolute",width:"70px",fontSize:"30px"});
},1000);},viewFile:function(a,c){var b='jax.call("community", "files,ajaxviewFiles","'+a+'","'+c+'")';cWindowShow(b,"",600,400);},ajaxdownloadFile:function(a,c){var b='jax.call("community","files,ajaxFileDownload","'+a+'","'+c+'")';
cWindowShow(b,"",600,200);},getFileList:function(e,c,b,a,d){joms.jQuery("#load-more-btn").html("");jax.loadingFunction=function(){};jax.call("community","files,ajaxgetFileList",e,c,b,a,d);
...
cWindowShow(b,"",600,200);},loadFile:function(d,c,b,a,e){joms.jQuery("#load-more-btn").html("");jax.loadingFunction=function(){};jax.call("community","files,ajaxgetFileList",d,c,b,a,e);
},searchFileUpdate:function(a,b){joms.jQuery(".cTabContainer ul").filter(":visible").html(a);},startdownload:function(a){var d="dl_"+(new Date).getTime();
a=a.replace("&amp;","&");joms.jQuery("<iframe>",{src:a,id:d}).appendTo("body");var c=document.getElementById(d);c=c.contentWindow||c.contentDocument;if(c.document){c=c.document;
}var b=setInterval(function(){if(c.readyState=="complete"){clearInterval(b);cWindowHide();}},1000);},},cover:{type:null,parentId:null,position:0,positionPercent:0,init:function(){joms.jQuery(".js-navbar-options, .js-navbar-icon").dropdown({toggle:false});
...
jax.call("community","photos,ajaxSetPhotoPhosition",joms.cover.type,joms.cover.parentId,0);},uploader:function(a){if(joms.cover.uploader.uploading){return;
}joms.cover.uploader.uploading=true;joms.jQuery.ajaxFileUpload({url:a,secureuri:false,fileElementId:"uploadCover",dataType:"json",beforeSend:function(){},complete:function(){joms.cover.uploader.uploading=false;
},success:function(c,b){if(c.success==true){joms.cover.updateCover(c.path,c.cancelbutton,c.savebutton);}if(c.error){if(!joms.jQuery("p#error_msg").is(":visible")){joms.jQuery("p#error_msg").show();
}joms.jQuery("p#error_msg").html(c.error);joms.jQuery("p#error_msg").fadeOut(7000);}}});},reposition:function(h,j,f,c){var a=joms.jQuery(".js-focus-change-cover");
a.data("display",a[0].style.display);a.hide();joms.cover.type=joms.jQuery(h).data("cover-context");joms.cover.parentId=j;joms.jQuery(".js-focus-gradient").hide();
var b=joms.jQuery("img#"+j).position().top;var g=joms.jQuery(".js-focus-cover").height()-joms.jQuery("img#"+j).height();var d='<button class="btn btn-primary" onClick="joms.cover.repositionCancel('+j+",'"+b+"')\"><span>"+f+'</span></button><button class="btn btn-primary ml-6" onClick="joms.cover.saveReposition()"><span>'+c+"</span></button>";
joms.jQuery(".span4.text-right").html(d);joms.jQuery("img#"+j).draggable({axis:"y",cursor:"move",drag:function(e,k){if(k.position.top<g){k.position.top=g;
}if(k.position.top>0){k.position.top=0;}},stop:function(l,m){var e=joms.jQuery("img#"+j),n=e.closest(".js-focus-cover"),k=m.position.top*100/n.height();
k=Math.round(k*10000)/10000;joms.cover.position=m.position.top;joms.cover.positionPercent=k;e.css("top",k+"%");}});},repositionCancel:function(c,b){var a=joms.jQuery(".js-focus-change-cover");
a.css({display:a.data("display")});joms.jQuery(".js-focus-gradient").show();joms.jQuery(".span4.text-right").html("");joms.jQuery("img#"+c).css({top:b+"px"});
joms.jQuery("img#"+c).draggable("destroy");},saveReposition:function(){jax.call("community","photos,ajaxSetPhotoPhosition",joms.cover.type,joms.cover.parentId,joms.cover.positionPercent+"%");
var a=joms.jQuery(".js-focus-change-cover");a.css({display:a.data("display")});joms.jQuery(".js-focus-gradient").show();joms.jQuery(".span4.text-right").html("");
joms.jQuery("img#"+joms.cover.parentId).draggable("destroy");joms.jQuery(".js-focus-change-cover a.btn.hidden").removeClass("hidden");}},share:{popup:function(a){joms.jQuery(document).off("click","#cWindowAction button.btn-primary");
